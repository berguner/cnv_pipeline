...
${ANNOTSV}/bin/AnnotSV -genomeBuild ${GENOME_BUILD} -SVinputFile ${UNION_BED} -outputFile ${ANNO_TSV} > ${FINAL_TSV}.log 2>&1;
awk 'BEGIN {FS = "\t"; OFS = "\t";} {
    if(NR == 1) {
...
${ANNOTSV}/bin/AnnotSV -genomeBuild ${GENOME_BUILD} -SVinputFile ${CODEX_BED} -outputFile ${ANNO_TSV} > ${FINAL_TSV}.log 2>&1;
awk 'BEGIN {FS = "\t"; OFS = "\t";} {
    if(NR == 1) {
    $6="CODEX2_pos\tCDX_type\tCDX_length(Kbp)\tCDX_raw_coverage\tCDX_normalized_coverage\tCDX_copy_number\tCDX_likelihood";
    print $0;
...
${ANNOTSV}/bin/AnnotSV -genomeBuild ${GENOME_BUILD} -SVinputFile ${EDP_BED} -outputFile ${ANNO_TSV} > ${FINAL_TSV}.log 2>&1;
awk 'BEGIN {FS = "\t"; OFS = "\t";} {
    if(NR == 1) {
