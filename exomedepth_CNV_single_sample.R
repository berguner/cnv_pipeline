...
args <- arg_parser(description='Run ExomeDepth CNV analysis for the given sample')
args <- add_argument(args, arg='--bed', default='/pipeline/resources/exome_cnv_calling_regions_b37.bed',
                     help='The path of the target region bed file')
args <- add_argument(args, arg='--project_folder', short='-p',
                     default='/pipeline/exome_cnv/',
...
args <- add_argument(args, arg='--sample_of_interest',
                     help='Name of the interesting sample')
...
case_sample <- p$sample_of_interest
